...
- 这里不仅要选对字段，还要选对日期比较语义
- 部分来源写入的 `publish_time_iso` 可能带有 `+08:00` 时区，而不是统一的 UTC 零时
- 因此“几月几号之前”的判断应优先按中国时区自然日解释，而不是直接按 UTC 零点切分
...
- 用户输入 `2025-12-31` 时，通常期待的是“丢弃 2025-12-31 之前的新闻”
- 这里的“之前”建议按 `Asia/Shanghai` 自然日解释
- 即比较“发布日期的中国时区日期部分”，而不是简单比较 UTC 时间戳
...
- 优先使用中国时区日期比较，例如 `publish_time_iso AT TIME ZONE 'Asia/Shanghai'`
- 或直接比较转换后的日期部分
- 不建议直接拿 `datetime.combine(date, midnight, tzinfo=UTC)` 做边界

否则会出现边界偏移：

- 例如 `2025-01-01 00:30 +08:00`
- 它对应 UTC 时间是 `2024-12-31 16:30`
- 如果按 UTC 零点比较，会被错误算进“2025-01-01 之前”
...
### 5. 计数展示要区分“桶总量”和“搜索命中”

当前左侧桶计数代表的是该桶全部待处理数量。

如果进入搜索模式，建议不要直接把左侧桶计数覆盖为搜索命中数，否则会让用户误以为桶总量变了。更稳妥的做法是：

- 左侧桶计数继续显示总量
- 工具栏附近单独显示“当前搜索命中 N 条”
...
如果实现时直接按 UTC 零点比较 `publish_time_iso`，会产生“某些凌晨新闻被算到前一天”的偏差。
...
- 按 `Asia/Shanghai` 自然日比较

### 2. `report_type` 作用域仍需明确

这里已经做出明确约束：

- 第一页固定只处理 `zongbao`
- 不给第一页补 `report_type` 切换

这能降低本次改造复杂度，但也带来一个已接受后果：

- 即使未来系统里存在 `wanbao` 数据，它也不在第一页新功能的处理范围内

如果后续业务要把 `wanbao` 也放进第一页，再单独扩展即可，不在本次范围内。

### 3. 搜索性能风险

如果直接在 `manual_reviews JOIN news_summaries` 的查询上做 `ILIKE title/summary/content`，在积压数据较大时可能变慢。

实现时应优先复用或对齐已有的 `news_summaries` 搜索表达式和索引策略，而不是临时拼一个全表扫描查询。

### 4. 控制台存在历史编码问题

部分模板和注释内容存在乱码痕迹。此次改造不建议顺手大面积清理编码问题，否则会扩大变更面。

### 5. `manual_filter_service.py` 仍承担兼容门面角色

这一轮不建议彻底重构为完整 package，但建议逐步把第一页查询、决策、聚类继续向明确模块边界靠拢。

### 6. 当前测试环境已恢复，但新增功能仍需补测试

当前本地 `venv` 和 `pytest` 已经恢复可用，现阶段可以直接执行：
...
这解决的是“测试环境不可用”的问题，但不代表新增功能可以跳过测试。日期筛选和桶内搜索仍然需要按本方案补服务层和查询层测试。
