...
- 系统设置页（`/admin/settings`，模板 `settings.html`）按页签拆为 `web_static/js/settings/`：`core.js`（共享状态、`apiRequest` 请求封装、分区保存的 409/422 公共流程、页签与 hash、未保存离开守卫）、`models_tab.js`、`sources_tab.js`、`accounts_tab.js`，启动在 `init.js`；样式在 `css/modules/settings.css`，面板/表格/按钮复用 `admin.css` 与 components 的现有类。页签体现在 URL hash：`#models` / `#sources` / `#accounts:来源key`（抓取账号页签内选中的来源也进 hash），切换页签只隐藏面板不重渲染，未保存修改靠这个保留。模型步骤的 `model` 为 `null` 表示「跟随默认」：界面上显示「跟随默认（当前：…）」并随默认模型输入框实时更新，保存时必须写回 `null` 而不是默认模型名；测试按钮一律用页面当前值（跟随默认时取默认输入框内容）。分区缺失（服务器未跑 `import-settings`）时页签只渲染导入提示、不渲染编辑控件。该页不是管理员主视图，不加载 `admin_last_view.js`。
