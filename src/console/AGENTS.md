...
- 存档条目修改模式：详情头部「修改」开关只给管理员渲染（角色经模板 body[data-user-role] 暴露，core.js 读为 isAdminUser），接口 `PATCH /api/submission-archive/items/{id}` 同样只放 admin。交互在 browser.js（toggleDetailEditMode / saveItemEdit / applyItemEditResult），可编辑 title/body/source/urls，表单样式复用录入编辑态（item_card.css）；保存成功后局部更新卡片展示区，不改回链字段、不重拉报告。
