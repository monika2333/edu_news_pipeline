...
## 实施清单（Checklist）
- 后端 API
  - [ ] `ExportRequest` 增加 `template`、`period`、`total_period`、`dry_run`、`mark_exported` 字段。
  - [ ] `export_batch` 支持综报/晚报两套模板（头部+栏目映射+符号/编号）。
  - [ ] 实现模板维度的期号自动计算（综报每日+1，晚报每日+2），前端传值则跳过计算。
  - [ ] `dry_run` 时仅返回文本，不写文件、不标记 exported。
  - [ ] 返回实际使用的 period/total/template，便于前端提示。

- 前端交互
  - [ ] 导出弹窗新增模板选择（综报/晚报）。
  - [ ] 期号输入框（period/total_period）默认填入后端建议值，可修改。
  - [ ] 预览开关：开启则 `dry_run=true`，禁用“标记已导出”。
  - [ ] “标记为已导出”勾选（默认开，预览时灰掉）。
  - [ ] 调用导出接口时携带模板、期号、dry_run、mark_exported；展示返回文本。

- 数据映射与排序
  - [ ] 综报栏目：重点关注舆情=京内负面，新闻信息纵览=京内正面，国内教育热点=京外负面；符号★/■/▲。
  - [ ] 晚报栏目：舆情速览=京内正面，舆情参考=京外正面；编号一二三…。
  - [ ] 排序沿用 manual_rank > score > 发布时间。
