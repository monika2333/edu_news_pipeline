...
1. 新 worker 位于 `src/workers/external_filter.py`（仿 `score.py:11`）：
   ...
   - 通过 `src/adapters/llm_scoring.py` 新增 `ExternalFilterScorer`，使用专用 prompt（写在 `docs/external_filter_prompt.md`）。
...
4. 在 docs/ 目录新增 external_filter_prompt.md，统一记录 external_filter prompt，便于协作与版本管理。
...
- [x] 起草 docs/external_filter_prompt.md 并敲定 external filter prompt。
- [ ] 实现 external_filter worker 与 LLM 评分逻辑（含阈值、重试、状态流转）。
- [ ] 更新 export SQL/brief writer，把 external_importance 元数据带入并展示。
