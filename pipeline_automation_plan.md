...
### 3. 结果推送到手机
- 使用已集成的飞书机器人：配置 FEISHU_APP_ID、FEISHU_APP_SECRET、FEISHU_RECEIVE_ID（支持旧版小写变量）后，export worker 会在成功生成简报后自动发送消息。
- 推送内容包含分类统计、前三条摘要以及导出文件路径；如需调整格式，可编辑 src/notifications/feishu.py。
- 若还需其他渠道（邮件、PushDeer 等），可以在同一模块中扩展多路通知逻辑。
- 自动任务和手动触发都会复用该通知机制，记得查看日志确认推送状态。
