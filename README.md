...
- LLM 业务模型：`LLM_SCORING_MODEL`、`LLM_SUMMARY_MODEL`、`LLM_SOURCE_MODEL`、`LLM_SENTIMENT_MODEL`、`LLM_EXTERNAL_FILTER_MODEL`、`LLM_BEIJING_GATE_MODEL`。
- LLM 思考默认策略：评分、来源识别、情感判断默认适合开启思考；摘要生成默认不思考。需要覆盖时可设置 `LLM_SUMMARY_REASONING_ENABLED`、`LLM_SOURCE_REASONING_ENABLED`、`LLM_SENTIMENT_REASONING_ENABLED`。
- LLM 超时：全局 `LLM_TIMEOUT`，或按步骤设置 `LLM_SCORING_TIMEOUT`、`LLM_SUMMARY_TIMEOUT`、`LLM_EXTERNAL_FILTER_TIMEOUT`、`LLM_BEIJING_GATE_TIMEOUT`。
