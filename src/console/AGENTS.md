...
- 反馈条目的已报送标签由 core.js 的 `priorMatchPill` 渲染，与回链 `linkPill` 完全独立，不复用 `.archive-link-pill` 的 is-* 修饰；三态：已报送/疑似已报送（偏绿色调，命中更早报送）与未报送（偏红色调，判定结束仍无命中）。「未报送」由 browser.js 的 `detailPriorMatchPill` 按报告级状态门控（`activeReportType === 'feedback' && !activeReportPriorMatchPending`），判定进行中不能误贴。点击标签打开命中明细弹窗（prior_matches.js + 模板 `#archive-prior-matches-modal`，样式在 item_card.css；未报送打开为空态）。详情轮询的继续条件除条目 `processing` 外还看 `report.prior_match_pending`（已报送判定在回链完成后才执行），仅剩它时受 `PRIOR_MATCH_POLL_LIMIT` 次数上限兜底；`item.prior_match` 与判定进度（pending）都必须纳入 `reportStatusSignature` 指纹，否则判定结果送达或「未报送」生效时页面不会重绘。detailStats 的「已报送」chip 仅反馈报告渲染、纯展示不参与筛选。prior_matches.js 与 manual_link.js 遵守同一约定：必须先于 content_drawer.js 加载，Escape 先关抽屉。
