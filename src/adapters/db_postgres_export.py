...
def get_manual_batch_by_tag(cur: psycopg.Cursor, report_tag: str) -> Optional[Dict[str, Any]]:
    query = """
        SELECT id, report_date, sequence_no, export_payload
...
def create_manual_batch(cur: psycopg.Cursor, report_tag: str) -> Dict[str, Any]:
    report_date, suffix = parse_report_tag(report_tag)
    fetch_query = """
        SELECT sequence_no
        FROM manual_export_batches
...
def get_manual_export_history(cur: psycopg.Cursor, report_tag: str) -> Tuple[Set[str], Optional[str]]:
    batch = get_manual_batch_by_tag(cur, report_tag)
    if not batch:
        return set(), None
    batch_id = str(batch["id"])
...
def record_manual_export(
    ...
    *,
    ...
    existing_ids, batch_id = get_manual_export_history(cur, report_tag)
    if batch_id is None:
        batch = create_manual_batch(cur, report_tag)
        batch_id = str(batch["id"])
    cur.execute(
...
def fetch_latest_manual_export_batch(cur: psycopg.Cursor) -> Optional[Dict[str, Any]]:
    query = """
        SELECT *
        FROM manual_export_batches
        ORDER BY report_date DESC, sequence_no DESC
...
def fetch_manual_export_items_by_batch(cur: psycopg.Cursor, batch_id: str) -> List[Dict[str, Any]]:
    query = """
        SELECT id, article_id, section, order_index, final_summary, metadata
        FROM manual_export_items
        WHERE manual_export_batch_id = %s
...
def fetch_manual_export_item_count(cur: psycopg.Cursor, batch_id: str) -> int:
...
__all__ = [
    ...
    "fetch_latest_manual_export_batch",
    "fetch_manual_export_item_count",
    "fetch_manual_export_items_by_batch",
    "get_all_exported_article_ids",
    "get_batch_by_tag",
    "get_export_history",
    "get_manual_batch_by_tag",
    "get_manual_export_history",
    "parse_report_tag",
    "record_export",
    "record_manual_export",
]
