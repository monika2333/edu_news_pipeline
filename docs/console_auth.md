...
The console API and manual filter page require authentication, configured via environment variables.

Set one (or both) of the following:

- `CONSOLE_BASIC_USERNAME` / `CONSOLE_BASIC_PASSWORD`: Enables HTTP Basic auth. Browsers will prompt for these credentials when visiting `/manual_filter` or the console root.
