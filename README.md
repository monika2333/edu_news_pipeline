...
- LLM 统一接入：`LLM_API_BASE_URL`、`LLM_API_KEY`，可选 `LLM_API_HTTP_REFERER`、`LLM_API_TITLE`。
- LLM 模型：默认只需设置 `LLM_MODEL`，所有 LLM 任务都会继承它；需要按任务覆盖时再设置 `LLM_SCORING_MODEL`、`LLM_SUMMARY_MODEL`、`LLM_SOURCE_MODEL`、`LLM_SENTIMENT_MODEL`、`LLM_EXTERNAL_FILTER_MODEL`、`LLM_BEIJING_GATE_MODEL`。
- LLM 思考默认策略：评分、来源识别、情感判断默认开启思考；摘要生成默认不思考。默认不传 `effort`，并设置 `exclude=true` 避免返回思考内容。需要覆盖时可设置 `LLM_REASONING_ENABLED`、`LLM_SUMMARY_REASONING_ENABLED`、`LLM_SOURCE_REASONING_ENABLED`、`LLM_SENTIMENT_REASONING_ENABLED`、`LLM_REASONING_EFFORT`、`LLM_REASONING_EXCLUDE`。
- LLM 超时：默认 90 秒；可用全局 `LLM_TIMEOUT` 覆盖，或按步骤设置 `LLM_SCORING_TIMEOUT`、`LLM_SUMMARY_TIMEOUT`、`LLM_EXTERNAL_FILTER_TIMEOUT`、`LLM_BEIJING_GATE_TIMEOUT`。
- 抓取/评分：如 `TOUTIAO_AUTHORS_PATH`、`TENCENT_AUTHORS_PATH`、`PROCESS_LIMIT`、`SCORE_PROMOTION_THRESHOLD` 等。
