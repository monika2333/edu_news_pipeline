...
- **新建 `src/adapters/http_laodongwubao.py`**
  ...
  - 输出统一结构：`article_id`, `title`, `content_markdown`, `publish_time_iso`, `source='laodongwubao'`（`source` 字段保持固定字符串 “劳动午报/laodongwubao”，不再使用作者信息；版面仅用于去重与日志，可在解析阶段丢弃，不写入数据库）。
...
- 新增 `scripts/run_ldwb_daily.ps1`：
  - 功能：每日一次执行 `crawl -> hash-primary -> score -> summarize -> external-filter`，触发源为劳动午报。
  - 关键实现：
