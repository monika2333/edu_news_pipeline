...
def run(limit: int = 50, *, concurrency: Optional[int] = None, keywords_path: Optional[Path] = None) -> None:
    ...
    if not candidates:
        print("No articles pending summarisation.")
        return
    ...
    if not filtered:
        print(f"All {len(candidates)} candidates filtered out by keyword rules.")
        return
    ...
    if workers == 1:
        for item in filtered:
            ...
            try:
                _process(item)
                success += 1
                print(f"OK {candidate.article_hash}")
    ...
    print(
        "done. ok={0} failed={1} filtered_out={2}".format(
            success,
            failed,
            skipped,
        )
