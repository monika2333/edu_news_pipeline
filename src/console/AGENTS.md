...
- 反馈条目的「已报送 / 疑似已报送」标签由 core.js 的 `priorMatchPill` 渲染（仅 `item.prior_match` 非空时输出），与回链 `linkPill` 完全独立，不复用 `.archive-link-pill` 的 is-* 修饰；点击打开命中明细弹窗（prior_matches.js + 模板 `#archive-prior-matches-modal`，样式在 item_card.css）。详情轮询的继续条件除条目 `processing` 外还看 `report.prior_match_pending`（已报送判定在回链完成后才执行），仅剩它时受 `PRIOR_MATCH_POLL_LIMIT` 次数上限兜底；`item.prior_match` 必须纳入 `reportStatusSignature` 指纹，否则判定结果送达页面也不会重绘。detailStats 的「已报送」chip 仅反馈报告渲染（报别再模块级 `activeReportType`）、纯展示不参与筛选。prior_matches.js 与 manual_link.js 遵守同一约定：必须先于 content_drawer.js 加载，Escape 先关抽屉。
