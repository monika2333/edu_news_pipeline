...
1. Apply schema changes (create `primary_articles`, extend `filtered_articles` and `news_summaries`, add indexes and triggers).
2. Update `crawl_sources.py` to write `filtered_articles` rows on keyword hits.
3. Build `hash_primary_worker` with hash utilities and SimHash integration.
4. Modify `score.py` to consume `primary_articles` and enforce the 60-point threshold.
5. Extend `summarize.py` for sentiment classification and new status transitions.
6. Update `export_brief.py` to emit four ordered buckets and rely on `sentiment_label`.
7. Adjust orchestration (scheduler, CLI commands) to run workers in the new order.
